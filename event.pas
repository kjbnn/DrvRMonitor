...
begin
  ...
  if 1 = 0 then
    case idIvent of
      ...
      $108:
        // Ничего. Пропуск не готового к постановке на охрану объекта
        begin
          mes.Code := R8_SH_BYPASS;
          st := Format('Пропуск ШС #%d [%.4d] %s', [Id, Sernum, PrmStr]);
        end;
      $109:
        // -в Задержка на вход. Переход физического ШС в состояние «Тревога», когда объект находится в состоянии «Взято» и для него определена задержка на вход.
        begin
      ...
      $10A:
        // +в Задержка на выход. Событие выдается при постановке объекта на охрану, если для него определена задержка на выход.
        begin
      ...
      $10B:
        // Ожидание готовности. При постановке на охрану, «Не готов»
        begin
      ...
      $201:
        // +т  Тревога. Переход физического ШС в состояние «Тревога», когда объект находится в со-стоянии «Норма».
        begin
          mes.Code := R8_SH_ALARM;
          st := Format('Тревога ШС #%d [%.4d] %s', [Id, Sernum, PrmStr]);
        end;
      $202:
        // +н  Неисправность. Переход физического ШС в состояние «Неисправность», когда объект находится в состоянии «Норма».
        begin
          mes.Code := R8_SH_CHECK;
      ...
      $204:
        // +г Готов к восстановлению. Переход физического ШС в состояние «Норма».
        begin
          mes.Code := R8_SH_READY;
      ...
      $205:
        // -г Не готов к восстановлению. Переход физического ШС в состояние «Тревога» или «Неисправность», когда объект находится в состоянии «Тревога». Переход физического ШС в состояние «Неисправность», когда объект находится в состоянии «Неисправность».
        begin
          mes.Code := R8_SH_NOTREADY;
      ...
      $402:
        // Область 1. Переход физического ШС в состояние в область 1. Разомкнуто для дискретных ШС
        begin
          mes.Code := R8_TECHNO_AREA1;
          st := Format('Область 1 ШС #%d [%.4d] %s', [Id, Sernum, PrmStr]);
        end;
      $403:
        // Неисправность. Переход физического ШС в состояние «Неисправность».
        begin
          mes.Code := R8_SH_CHECK;
          st := Format('Неисправность ШС #%d [%.4d] %s', [Id, Sernum, PrmStr]);
        end;
      $404:
        // Тревожная область 0. Переход физического ШС в состояние в область 0, область 0 сконфигурирована как тревожная
        begin
          // исключение
          mes.Code := R8_TECHNO_AREA0;
          // send(mes);
      ...
      $405:
        // Тревожная область 1. Переход физического ШС в состояние в область 1, область 1 сконфигурирована как тревожная
        begin
          // исключение
          mes.Code := R8_TECHNO_AREA1;
          // send(mes);
      ...
      $406:
        // Область 2. Переход физического ШС в состояние в область 2
        begin
          mes.Code := R8_TECHNO_AREA2;
          st := Format('Область 2 ШС #%d [%.4d] %s', [Id, Sernum, PrmStr]);
        end;
      $407:
        // Область 3. Переход физического ШС в состояние в область 3
        begin
          mes.Code := R8_TECHNO_AREA3;
          st := Format('Область 3 ШС #%d [%.4d] %s', [Id, Sernum, PrmStr]);
        end;
      $408:
        // Тревожная область 2. Переход физического ШС в состояние в область 2, область 2 сконфигурирована как тревожная
        begin
          // исключение
          mes.Code := R8_TECHNO_AREA2;
          // send(mes);
      ...
      $409:
        // Тревожная область 3. Переход физического ШС в состояние в область 3, область 3 сконфигурирована как тревожная
        begin
          // исключение
          mes.Code := R8_TECHNO_AREA3;
          // send(mes);
      ...
      $603:
        ...
        begin
          ...
          st := Format(' Проход по команде «Открыть замок» ТД #%d [%.4d]. %s',
            [Id, Sernum, PrmStr]);
        end;
      ...
      $60C: // Ошибка авторизации
        begin
          ...
          begin
            ...
            st := Format('Доступ запрещен. Неверный пинкод. ТД #%d [%.4d]. %s',
              [Id, Sernum, PrmStr]);
          end;
        end;
      $60D: // подбор (!)
      ...
      $704:
        // Попытка подбора кода. Событие выдается после трех, сделанных подряд, ошибок авто-ризации пользователя.
        begin
      ...
      $80E:
        begin
          st := Format('Ошибка авторизации пользователя. ШЛЮЗ #%d [%.4d]. %s',
            [Id, Sernum, PrmStr]);
        end;
      $80F:
        begin
      ...
      $90C:
        begin
          ...
          st := Format('Ошибка авторизации пользователя. АСПТ #%d [%.4d]. %s',
            [Id, Sernum, PrmStr]);
        end;
      $90D:
        begin
      ...
      $912:
        begin
          ...
          st := Format('Срабатывание датчика наличия ОТВ. АСПТ #%d [%.4d]. %s',
            [Id, Sernum, PrmStr]);
        end;
      $913:
        begin
      ...
      $2001:
        begin
          ...
          st := Format('Потеря связи с СУ %d:%d', [idBcp, Sernum, PrmStr]);
        end;

      $2002:
        begin
          mes.Code := R8_CU_CONNECT_ON;
          st := Format('Восстановление связи с СУ %d:%d', [Id, Sernum, PrmStr]);
        end;

      $3F01:
        begin
          mes.Code := R8_CONNECT_FALSE;
          st := Format('Рубеж Сервер. Потеря связи с БЦП %d',
            [Id, Sernum, PrmStr]);
        end;
      ...
      $3F02:
        begin
          ...
          st := Format('Рубеж Сервер. Восстановление связи с БЦП %d',
            [Id, Sernum, PrmStr]);
        end;
